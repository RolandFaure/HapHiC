...
def export_to_bam(segments, bamFile, newnames):
    ...
    for s in segments:
        for i, n in enumerate(s.names):
            if n in names:
                ...
                lengths_supercontigs[n] = np.sum([s.lengths[j] for j in range(len(s.lengths))])
                length_contigs[n] = s.lengths[i]
    ...
    already_written = {}
    for n in names:
        if names[n][2] not in already_written:
            head.append({"LN": lengths_supercontigs[n], "SN": names[n][2]})
            already_written[names[n][2]] = len(head) - 1
        indices[n] = already_written[names[n][2]]
    ...
    for read in oldfile.fetch(until_eof=True):
        ...
        if contig in names:
            ...
            if names[contig][0] == "+":
                ...
                a.reference_start = length_contigs[contig] - start + int(names[contig][0])

            if read.next_reference_name in names:
                a.next_reference_id = indices[read.next_reference_name]
                if names[read.next_reference_name][0] == "+":
                    a.next_reference_start = read.next_reference_start + int(names[read.next_reference_name][0])
                else:
                    a.next_reference_start = length_contigs[read.next_reference_name] - read.next_reference_start + int(names[read.next_reference_name][0])
