...
def bridge_with_long_reads(
    ...
    supported_links = sparse.lil_matrix(
        (len(names) * 2, len(names) * 2)
    )  # supported links is the list of the links between different contigs found in the gaf file

    lines = []
    if ".gaf" in gafFile:
        logging.info("Reading the gaf file...")
        read_GAF(gafFile, 0, 0, lines)
        logging.info("Finished going through the gaf file.")
    elif ".tsv" in gafFile:
        logging.info("Reading the tsv file...")
        read_TSV(gafFile, names, lines)
        logging.info("Finished going through the tsv file.")
    else:
        logging.error(
            "ERROR: input format of mapped read not recognized. It should be .gaf (from GraphAligner) or .tsv (from SPAligner)"
        )
        sys.exit(1)

    # determine an approximate list of contigs that look haploid
    haploidContigs, haploidContigsNames = determine_haploid_contigs(
        lines, segments, names
    )
    ...
    longContigs = (
        []
    )  # long contigs are contigs too long to be traversed by a long read in the gaf
    longContigs = [
        True for i in range(len(names))
    ]  # then all contigs that are in the middle of a read will be marked as False
    bridges = [
        [[], []] for i in range(len(haploidContigs))
    ]  # bridges is a list inventoring at index haploidCOntigsNames[seg.names[0]] all the links left and right of the contig, supported by the gaf
    minimum_supported_links = sparse.lil_matrix(
        (len(names) * 2, len(names) * 2)
    )  # minimum_supported links is the list of all links between different contigs found at least once in the gaf file
    inventoriate_bridges(
        lines,
        bridges,
        minimum_supported_links,
        haploidContigsNames,
        longContigs,
        names,
        segments,
    ...
    # logging.info("Bridge 1756 : ", bridges[haploidContigsNames['1756']])
        ...
        for s in segments:
            ...
            for end in range(2):
                ...
                while index < len(s.links[end]):
                    if (
                        minimum_supported_links[
                            2 * names[s.names[0]] + end,
                            2 * names[s.links[end][index].names[0]]
                            + s.otherEndOfLinks[end][index],
                        ]
                        == 0
                    ):
                        sg.delete_link(
    ...
    consensus_bridges = [
        ["", ""] for i in range(len(haploidContigs))
    ]  # consensus bridge is essentially the same as bridges, except there is only one bridge left at each side for each contig
    logging.info("Building consensus bridges from all the long reads")

    haploidContigs, haploidContigsNames, consensus_bridges = build_consensus_bridges(
        consensus_bridges, bridges, names, haploidContigs, haploidContigsNames
    )
    logging.info("Done building consensus bridges                 ")

    # logging.info("contig 1756 in haploid : ", "1756" in haploidContigsNames)
    # logging.info("contig 93 consensus : ", consensus_bridges[haploidContigsNames["93"]])
    # logging.info("contig 11 consensus : ", consensus_bridges[haploidContigsNames["11"]])
    # logging.info("contig 2601 consensus : ", consensus_bridges[haploidContigsNames["2601"]])
    bridges = []

    # logging.info("contig 121 in haploid : ", "121" in haploidContigsNames)
    ...
    while (
        not sure_haploids
    ):  # knowing what contigs are really haploid may take several iterations
        leng = len(haploidContigs)

        # logging.info("consensus of 20: ", consensus_bridges[haploidContigsNames['20']])
        ...
        # logging.info("nonoverlap consensus of 20: ", non_overlapping_bridges[haploidContigsNames['20']])
        ...
        if not sure_haploids:
            logging.info(
                "Out of ",
                leng,
                " supposed single-copy contigs, ",
                leng - len(haploidContigs),
                " were not actually haploid. Recomputing until all the single-copy contigs are robust",
            )

    # logging.info("contig 154 in haploid : ", "154" in haploidContigsNames)

    # from the consensus bridges, mark all links that are supported by the long reads
    supported_links = sparse.lil_matrix(
        (len(names) * 2, len(names) * 2)
    )  # supported links is the list of the links between different contigs found in the gaf file, and in how many different consensus
    compute_supported_links(
        supported_links,
        non_overlapping_bridges,
        haploidContigsNames,
        haploidContigs,
    ...
    # logging.info("Link is supported after maxing with strength ", supported_links2[2*names['127'], 2*names['112']])
    # logging.info("Link is supported after maxing with strength ", supported_links2[2*names['2505'], 2*names['2489']])

    # logging.info("Contig 15 has for consensus : ", consensus_bridges[haploidContigsNames["15"]])

    # now actually unzip the graph using the instructions in non_overlapping_bridges

    logging.info("Let's move on to actually untangling the graph")
    unzip_graph_with_bridges(
        segments,
        non_overlapping_bridges,
        copiesnumber,
        haploidContigs,
    ...
    # logging.info(non_overlapping_bridges)
...
def determine_haploid_contigs(lines, segments, names):
    ...
    ]  # list of neighbor contigs left and right of each contig : if a contig has only one neighbor left and right we'll say it's haploid
    ...
    for line in lines:
        ...
        for c, contig in enumerate(contigs):
            ...
            if c < len(contigs) - 1:
                if (
                    contigs[c + 1]
                    not in neighborLeftRight[names[contig]][1 - orientation]
                ):
                    neighborLeftRight[names[contig]][1 - orientation][
                        contigs[c + 1]
                    ] = 1
                else:
                    neighborLeftRight[names[contig]][1 - orientation][
    ...
    for se, nei in enumerate(neighborLeftRight):
        if (
            segments[se].length > 100
        ):  # that's because too short contigs cause trouble (in great part because erroneous contigs are often very short)
    ...
    for l, line in enumerate(lines):
        ...
        for c, contig in enumerate(contigs):
            if c > 0:
                ...
                if -1 == sg.find_this_link(
                    segments[names[contig]],
                    or2,
                    segments[names[contigs[c - 1]]].links[or1],
                    segments[names[contigs[c - 1]]].otherEndOfLinks[or1],
                ):
                    logging.warning(
                        "WARNING: discrepancy between what's found in the alignment files and the inputted GFA graph. Link ",
                        contigs[c - 1 : c + 1],
                        orientations[c - 1 : c + 1],
                        " not found in the gfa",
                    )
                    possible = False
        ...
        if possible:
            for c, contig in enumerate(contigs):
                if c > 0:
                    ...
                    minimum_supported_links[
                        2 * names[contigs[c - 1]] + or1, 2 * names[contigs[c]] + or2
                    ] = 1
                    minimum_supported_links[
                        2 * names[contigs[c]] + or2, 2 * names[contigs[c - 1]] + or1
                    ] = 1
                ...
                if contig in haploidContigsNames:
                    if orientations[c] == ">":
                        ...
                        for c2 in range(c + 1, len(contigs)):
                            bridges[haploidContigsNames[contig]][1][-1] += (
                                orientations[c2] + contigs[c2]
                            )
                        ...
                        for c2 in range(c - 1, -1, -1):
                            if orientations[c2] == ">":
                                bridges[haploidContigsNames[contig]][0][-1] += (
                                    "<" + contigs[c2]
                                )
                            else:
                                bridges[haploidContigsNames[contig]][0][-1] += (
...
def build_consensus_bridges(
    consensus_bridges, bridges, names, haploidContigs, haploidContigsNames
):
    not_actually_haploid = (
        []
    )  # a list of not actually haploid contigs : at this point, we will rule out as haploid contigs those that consensus back on themselves

    # logging.info(allcontigs[0][1])
    ...
    for c in range(len(bridges)):
        ...
        localOrientations = [
            [
                "".join(re.findall("[<>]", bridges[c][j][k]))
                for k in range(len(bridges[c][j]))
            ]
            for j in range(2)
        ]
        ...
        for end in range(2):  # look left and right of each contig
            kept_reads = [
                i for i in range(len(bridges[c][end])) if bridges[c][end][i] != ""
            ]  # list keeping in mind all the reads still in the consensus
            pos = 0
            consensus = ""
            ...
            while len(kept_reads) > 0:
                ...
                #     logging.info("Consensuing : ", Counter(candidate2))

                if (
                    cons1[1] > 0
                    and (
                        cons1[1] > 0.6 * len(candidate1)
                        or (cons1[1] >= 2 and cons1[1] == len(candidate1) - 1)
                    )
                    and (
                        cons2[1] > 0.6 * len(candidate1)
                        or (cons2[1] >= 2 and cons2[1] == len(candidate1) - 1)
                    )
                ):  # consensus is defined there as a 60% of the propositions agreeing or only 1 proposition disagreeing
                    if cons1[0] == "*":
                        break

                    consensus_bridges[c][end] = (
                        consensus_bridges[c][end] + cons1[0] + cons2[0]
                    )
                    ...
                    # logging.info('Adding from ', consensus_bridges[c][end], " c: ", c)
                    ...
                    for r in kept_reads:
                        if len(localContigs[end][r]) > pos + 1:
                            if (
                                localOrientations[end][r][pos] == cons1[0]
                                and localContigs[end][r][pos] == cons2[0]
                            ):
                                new_kept += [r]
                    ...
                    # if "11" in haploidContigs[c].names :
                    #     logging.info("There are no consensus there: ", candidate1, candidate2)
                    break
...
def compute_supported_links(
    ...
    for c in range(len(consensus_bridges)):
        for end in range(2):
            ...
            previousContig = names[
                haploidContigs[c].names[0]
            ]  # variable containing the index of the last contig of the consensus (useful to fill supported_links)
            ...
            if len(contigs) > 0:
                if longContigs[
                    names[contigs[-1]]
                ]:  # it's a long contig, a bridge will be built
                    firstHapIdx = len(contigs) - 1

            # if '7200' in contigs :
            #     logging.info("Bridge from 433 will look like : ", contigs[:firstHapIdx+1], " ", firstHapIdx)
            ...
            for co in range(firstHapIdx + 1):
                ...
                supported_links[
                    2 * names[contig] + current_end, 2 * previousContig + previousEnd
                ] += 1  # each link is present twice in supported_links, only add to one cell of supported_links, the symetric one can be added from the other end of the bridge
                symmetricSupport[
                    2 * previousContig + previousEnd, 2 * names[contig] + current_end
                ] += 1
                # if (contig == "7200" and names['4796'] == previousContig) or (contig == "4796" and names['7200'] == previousContig) :
                #     logging.info("Increasing the link : ", haploidContigs[c].names[0], ",", consensus_bridges[c][end])
                if non_overlapping:
                    symmetricSupport[
                        2 * names[contig] + current_end,
                        2 * previousContig + previousEnd,
                    ] += 1
                    supported_links[
                        2 * previousContig + previousEnd,
                        2 * names[contig] + current_end,
    ...
    if not non_overlapping:
        ...
        for r, c, m in zip(
            symmetricSupport.row, symmetricSupport.col, symmetricSupport.data
        ):
            supported_links[r, c] = max(supported_links[r, c], m)

    # logging.info("Strength of link : ", supported_links[2*names['4796'], 2*names['7200']])
...
def merge_bridges(
    ...
    not_actually_haploid = (
        []
    )  # a list of not actually haploid contigs among the haploidContigs
        ...
        for end in range(2):
            if consensus_bridges[c][end] != "":
                ...
                if firstHapIdx >= 0:  # in case there is a full bridge
                    ...
                    symmetrical = re.split(
                        "[><]",
                        consensus_bridges[haploidContigsNames[contigs[firstHapIdx]]][
                            otherEnd
                        ],
                    )

                    if (
                        len(symmetrical) > firstHapIdx + 1
                        and haploidContigs[c].names[0] == symmetrical[firstHapIdx + 1]
                    ):  # +1 because the first element of symmetrical is always ''
                        coherent = True
                    ...
                    if not coherent:
                        ...
                        if (
                            len(symmetrical) <= firstHapIdx + 1
                        ):  # means the symmetrical did not manage to consensus
                            # if '127' == contigs[firstHapIdx] :
                            #     logging.info("two incoherent bridges between ", haploidContigs[c].names[0] , " and ", haploidContigs[haploidContigsNames[contigs[firstHapIdx]]].names[0], " : ", consensus_bridges[c][end], " vs ", consensus_bridges[haploidContigsNames[contigs[firstHapIdx]]][otherEnd])
                            not_actually_haploid += [
                                haploidContigsNames[contigs[firstHapIdx]]
                            ]
                            sure_haploids = False

                            # logging.info("Contig ", contigs[firstHapIdx], " does not look haploid, seen from contig ",  haploidContigs[c].names[0], " : ", contigs[:firstHapIdx+1], ", whose bridge is ", symmetrical[:firstHapIdx+2])
                        ...
                        #         logging.info("Contig ", haploidContigs[c].names[0], " does not look haploid, here is its bridge to the next contig ", contigs[firstHapIdx] , " : ", contigs[:firstHapIdx+1], ", whose bridge is ", symmetrical[:firstHapIdx+2])
                        #     #logging.info("Haploid contigs : ", haploidContigsNames)

                    else:
                        if (
                            c < haploidContigsNames[contigs[firstHapIdx]]
                        ):  # keep only one of the two bridges
                            non_overlapping_bridges[c][end] = "".join(
                                [
                                    orientations[i] + contigs[i]
                                    for i in range(firstHapIdx + 1)
                                ]
                            )

                elif longContigs[
                    names[contigs[-1]]
                ]:  # if it ends on a  multiploid long contig, extracting could still be done (though not for the last, long contig). That's because we know there is no bridge going the other direction through the long contig
...
def unzip_graph_with_bridges(
    ...
    for s, seg in enumerate(segments):
        ...
        for n, neighbor in enumerate(seg.links[0]):
            minLeft += supported_links[
                2 * names[seg.names[0]],
                2 * names[neighbor.names[0]] + seg.otherEndOfLinks[0][n],
            ]
        for n, neighbor in enumerate(seg.links[1]):
            minRight += supported_links[
                2 * names[seg.names[0]] + 1,
                2 * names[neighbor.names[0]] + seg.otherEndOfLinks[1][n],
            ]
    ...
    # logging.info("Multiplicities of contig 127 : ", multiplicities[names['127']])

    alreadyDuplicated = [
        -1 for i in range(len(names))
    ]  # list useful for duplicating long contigs : only duplicate them from one side, the one that is in this list
    ...
    for se in range(l):
        ...
        if s.names[0] in haploidContigsNames:
            for end in range(2):
                if (
                    len(non_overlapping_bridges[haploidContigsNames[s.names[0]]][end])
                    > 0
                ):  # and ('358' in non_overlapping_bridges[haploidContigsNames[s.names[0]]][end] or '339' in non_overlapping_bridges[haploidContigsNames[s.names[0]]][end]): #means there is a bridge to be built there
                    contigs = re.split(
                        "[><]",
                        non_overlapping_bridges[haploidContigsNames[s.names[0]]][end],
                    )
                    orientations = "<>"[end] + "".join(
                        re.findall(
                            "[<>]",
                            non_overlapping_bridges[haploidContigsNames[s.names[0]]][
                                end
                            ],
                        )
                    )
                    ...
                    newContigsIndices = [
                        names[contigs[0]]
                    ]  # list keeping track of the indices of the contigs on the path
                    oldContigsIndices = [names[i] for i in contigs]

                    # if '6897' in contigs :
                    #     logging.info("Unzipping contig ", s.names[0], " with bridge : ",contigs)

                    haploidCoverage = (
                        segments[names[contigs[0]]].depths[0]
                        + segments[names[contigs[-1]]].depths[0]
                    ) / 2  # computation of the haploid coverage at this point in the assembly

                    nextEnd = 0
                    if orientations[1] == "<":
                        nextEnd = 1
                    CIGAR = segments[names[contigs[0]]].CIGARs[end][
                        sg.find_this_link(
                            segments[oldContigsIndices[1]],
                            nextEnd,
                            segments[names[contigs[0]]].links[end],
                            segments[names[contigs[0]]].otherEndOfLinks[end],
                        )
                    ]
                    nextCIGAR = ""
                    ...
                    #     #logging.info('here edge 291 neighbors are ', [i.names for i in segments[names['edge_291']].links[0]], " ", contigs)
                    ...
                    # logging.info('now edge 291 neighbors are ', [i.names for i in segments[names['edge_291']].links[0]], " ", contigs)
                    ...
                    for c in range(1, len(contigs)):
                        ...
                        # if contigs[c] == '7200' :
                        #     logging.info("Link supported with strength : ", supported_links[2*names["7200"], 2*names["4796"]])
                        # logging.info("Using once contig 7200 in overlap ", non_overlapping_bridges[haploidContigsNames[s.names[0]]][end], ", now at : ", multiplicities[names[contigs[c]]])
                        ...
                        # logging.info(alreadyDuplicated[names['edge_291']])
                        ...
                        if c < len(contigs) - 1:
                            ...
                            if (
                                sg.find_this_link(
                                    segments[oldContigsIndices[c + 1]],
                                    nextEnd,
                                    contig.links[1 - end1],
                                    contig.otherEndOfLinks[1 - end1],
                                )
                                != -1
                            ):
                                nextCIGAR = contig.CIGARs[1 - end1][
                                    sg.find_this_link(
                                        segments[oldContigsIndices[c + 1]],
                                        nextEnd,
                                        contig.links[1 - end1],
                                ...
                                logging.debug(
                                    ...
                                    contigs,
                                    " : looking for ",
                                    segments[oldContigsIndices[c + 1]].names,
                                    " ",
                                    nextEnd,
                                    " from ",
                                    contig.names,
                                    " among ",
                                    [i.names for i in contig.links[1 - end1]],
                                    " ",
                                    contig.otherEndOfLinks[1 - end1],
                                    " ",
                                    s.names[0],
                                    " ",
                                    non_overlapping_bridges[
                                        haploidContigsNames[s.names[0]]
                                    ][end],
                                )

                        # logging.info("Link supported with strength ", supported_links[names[contigs[c]*2+end1 , names[contigs[c-1]]*2+end0])

                        # if s.names[0] == '55374' and end == 0 and contigs[c] == '2505' :
                        #     logging.info("multiplicity of 2505 : " , multiplicity)

                        if (
                            multiplicity > 1 and c < len(contigs) - 1
                        ):  # if multiplicity>1, the contig should be duplicated
                            newSegment = sg.Segment(
                                contig.names,
                                contig.orientations,
                                contig.lengths,
                                contig.insideCIGARs,
                                HiCcoverage=contig.HiCcoverage,
                                readCoverage=[i / multiplicity for i in contig.depths],
                            )
                            ...
                            supported_links[
                                names[contigs[c]] * 2 + end1,
                                names[contigs[c - 1]] * 2 + end0,
                            ] -= 1
                            supported_links[
                                names[contigs[c - 1]] * 2 + end0,
                                names[contigs[c]] * 2 + end1,
                            ...
                            if (
                                ...
                                == 0
                                and len(segments[oldContigsIndices[c - 1]].links[end0])
                                > 1
                                and sg.find_this_link(
                                    segments[oldContigsIndices[c]],
                                    end1,
                                    segments[oldContigsIndices[c - 1]].links[end0],
                                    segments[oldContigsIndices[c - 1]].otherEndOfLinks[
                                        end0
                                    ],
                                    warning=False,
                                )
                                != -1
                                and len(segments[oldContigsIndices[c]].links[end1]) > 1
                            ):  # be sure not to create dead ends
                                # if "130" in segments[oldContigsIndices[c-1]].names :
                                #     logging.info("remove links left of ", segments[oldContigsIndices[c]].names, " : ", segments[oldContigsIndices[c-1]].names)
                                sg.delete_link(
                                    segments[oldContigsIndices[c]],
                                    end1,
                                    segments[oldContigsIndices[c - 1]],
                                    end0,
                                    warning=True,
                                )  # though it is very hard to be sure, it is not impossible at that point that we actually delete a link that is present in another bridge, so don't warn
                            ...
                            #     logging.info("New 3682 : ", non_overlapping_bridges[haploidContigsNames[s.names[0]]][end])
                            #     logging.info([s.names for s in segments[-1].links[0]])
                            #     logging.info([s.names for s in segments[-1].links[1]])
                            ...
                            if (
                                segments[newContigsIndices[c - 1]]
                                not in contig.links[end1]
                            ):  # the link could have been already kept to prevent dead ends
                                sg.add_link(
                                    contig,
                                    end1,
                                    segments[newContigsIndices[c - 1]],
                            ...
                            if len(contig.links[end1]) > 0:
                                for n, neighbor in enumerate(contig.links[end1]):
                                    if (
                                        neighbor.ID
                                        != segments[newContigsIndices[c - 1]].ID
                                    ):
                                        # then delete the links, except if they are still supported or create dead ends
                                        if (
                                            supported_links[
                                                names[contigs[c]] * 2 + end1,
                                                names[neighbor.names[0]] * 2
                                                + contig.otherEndOfLinks[end1][n],
                                            ]
                                            <= 0
                                            and minimum_supported_links[
                                                names[contigs[c]] * 2 + end1,
                                                names[neighbor.names[0]] * 2
                                                + contig.otherEndOfLinks[end1][n],
                                            ]
                            ...
                            # logging.info("RIght of 138 there is ", [i.ID for i in segments[names['138']].links[1]], " ", [i for i in segments[names['119']].otherEndOfLinks[1]], " ", segments[names['119']].ID)
                            # logging.info("RIght of 119 there is ", [i.ID for i in segments[names['119']].links[1]], " ", [i for i in segments[names['138']].otherEndOfLinks[1]], " ", segments[names['138']].ID)
                            if len(contig.links[1 - end1]) > 0 and c < len(contigs) - 1:
                                ...
                                if not (
                                    c == len(contigs) - 2
                                    and alreadyDuplicated[names[contigs[c + 1]]]
                                    == 1 - nextEnd
                                    and contigs[c + 1] not in haploidContigs
                                ):  # delete all the links right of the contig, the only good one will be reestablished later, except if looking at a long contig duplicated from the other end
                                    idx = 0
                                    while len(contig.links[1 - end1]) > idx:
                                        ...
                                        if (
                                            ...
                                            > 1
                                        ):  # you don't want to create dead ends
                                            sg.delete_link(
                                                contig,
                                                1 - end1,
                                                neighbor,
                                                contig.otherEndOfLinks[1 - end1][idx],
                                            )
                                        else:
                                            idx += 1
                    ...
                    # logging.info('edge 291 neighbors are ', [i.names for i in segments[names['edge_291']].links[0]], " ", contigs)
                # if supported_links[names['2484']*2+1 , names['2505']*2] == 1:
                #     logging.info(non_overlapping_bridges[haploidContigsNames[s.names[0]]][end], "  ", [i.names for i in segments[names['2484']].links[1]], " ", s.names[0], " ", end)
                #     while True :
                #         rien = 0
...
def trim_tips(
    segments, multiplicities, names, haploidContigsNames, supported_links, exhaustive
):
            ...
            if len(seg.names) == 1:  # our only target here is misassembled contigs
                ...
                for end in range(2):
                    ...
                    for n, neighbor in enumerate(seg.links[end]):
                        ...
                        if (
                            supported_links[
                                2 * names[seg.names[0]] + end,
                                2 * names[neighbor.names[0]] + otherEnd,
                            ]
                            == 1
                        ):  # this is supported
                            someSupported = True
                        if (
                            len(neighbor.links[otherEnd]) == 1
                        ):  # let's not create extra dead ends
                            someSupported = True
    ...
    for s, seg in enumerate(segments):
        for end in range(2):
            if (
                len(seg.links[1 - end]) == 0
                and len(seg.links[end]) == 1
                and seg.length < 2000
            ):  # if this contig is a very short dead end, delete it
                neighbor = seg.links[end][0]
                neighborEnd = seg.otherEndOfLinks[end][0]
                # logging.info("Checking if ", seg.names, " is a tip")
                if any(
                    [
                        extended_length(
                            i,
                            neighbor.otherEndOfLinks[neighborEnd][e],
                            10 * seg.length,
                            30,
                        )
                        for e, i in enumerate(
                            seg.links[end][0].links[seg.otherEndOfLinks[end][0]]
                        )
                    ]
                ):  # this means we're in a very short dead end
                    if all(
                        [i not in haploidContigsNames for i in seg.names]
                    ):  # then it means it's probably an error in the determination of the multiplicity
                        sg.delete_link(
                            seg, end, seg.links[end][0], seg.otherEndOfLinks[end][0]
                        )
                        toDelete += [s]

            if (
                len(seg.links[1 - end]) == 0 and len(seg.links[end]) == 1
            ):  # if this contig is a replica of a parallel contig, merge both together
                neighbor = seg.links[end][0]
                neighborEnd = seg.otherEndOfLinks[end][0]
                ...
                for n2, neighbor2 in enumerate(neighbor.links[neighborEnd]):
                    ...
                    if (
                        neighbor2.length > seg.length
                    ):  # if we're not looking at the same contig
                        # see if it's parallel to the dead end, i.e. has the same contigs in the same order
                        same = True
                        for co in range(len(seg.names)):
                            nameParallel1 = neighbor2.names[
                                otherEnd2 * (len(neighbor2.names) - 1)
                                + (-2 * otherEnd2 + 1) * co
                            ]
                            nameParallel2 = seg.names[
                                end * (len(seg.names) - 1) + (-2 * end + 1) * co
                            ]
                        ...
                        if same:
                            # logging.info("Found two parallels contigs : ", seg.names, neighbor2.names)
...
def extended_length(segment, end, thresholdLength, thresholdContigs):
    # logging.info("Extended length called with threshold ", thresholdLength, " on segment , ", sg.names)
    ...
    # logging.info("Longest contigs : ", longestContig, [segment.links[1-end][i].length for i in longestContig])

    for n in longestContig[
        : min(len(longestContig), 2)
    ]:  # only explore the 2 most promising neighbors, beyond it's not worth it
        neighbor = segment.links[1 - end][n]
        if extended_length(
            neighbor,
            segment.otherEndOfLinks[1 - end][n],
            thresholdLength - segment.length,
            thresholdContigs - 1,
        ):
            return True
